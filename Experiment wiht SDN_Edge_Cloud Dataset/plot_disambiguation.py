...
def plot_fold(outdir, fold, dataset_events_json, out_png=None, margin=40, title=None):
    ...
    # New IEEE-friendly palette
    C_CAUSE   = "#E67E22"   # orange
    C_VICTIM  = "#16A085"   # teal
    C_GATE_C  = "#D35400"   # dark orange
    C_GATE_V  = "#117864"   # dark teal
    C_DRIFT   = "#D6DBDF"   # light gray
    ...
    if victim:
        ax1.plot(
            ...
            color=C_VICTIM,
            lw=3.0,
            label='Victim Risk Score'
        )

    ax1.set_ylabel('Smoothed Risk Score', fontsize=20); ax1.set_ylim(-0.02, 1.05)
    ax1.tick_params(axis='both', labelsize=14)
    ax1.grid(True, ls='-.', lw=0.5, alpha=0.6)

    if th.get(cause, {}).get('tau') is not None:
        ax1.hlines(th[cause]['tau'], d['t'].iloc[0], d['t'].iloc[-1], colors=C_CAUSE,  linestyles=(0,(5,3)), lw=2, label='Alarm Threshold (Cause)')
    if victim and th.get(victim, {}).get('tau') is not None:
        ax1.hlines(th[victim]['tau'], d['t'].iloc[0], d['t'].iloc[-1], colors=C_VICTIM, linestyles=(0,(5,3)), lw=2, label='Alarm Threshold (Victim)')
    ...
    ax1.axvspan(
        ...
        alpha=0.35,
        hatch='///',
        label='Failure Propagation Window'
    )
    ...
    if gate:
        ax2 = ax1.twinx()
        if cause in gate:
            ax2.plot(d['t'], gate[cause][(df['t'] >= t0) & (df['t'] <= t1)],  color=C_CAUSE,  ls='-.', lw=2, label='Gating Probability (Cause)')
        if victim and victim in gate:
            ax2.plot(d['t'], gate[victim][(df['t'] >= t0) & (df['t'] <= t1)], color=C_VICTIM, ls='-.', lw=2, label='Gating Probability (Victim)')
        ax2.set_ylabel('Gating Probability', fontsize=20); ax2.set_ylim(-0.02, 1.05)
        ax2.tick_params(axis='y', labelsize=14)

    ax1.set_xlabel('Time (minutes)', fontsize=20)
    ax1.set_ylabel('Smoothed Risk Score', fontsize=20)
    #ax1.set_title(title or f"Fold {fold}: {e.get('type','event')}")
    handles, labels = ax1.get_legend_handles_labels()
    if 'ax2' in locals():
        h2, l2 = ax2.get_legend_handles_labels()
        handles += h2; labels += l2
    ax1.legend(handles, labels, loc='upper left', fontsize=14, frameon=True,  framealpha=0.3)
