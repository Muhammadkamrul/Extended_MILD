...
def _stage_label(active_horizons):
    active_horizons = sorted(set(int(h) for h in active_horizons), reverse=True)

    if len(active_horizons) == 0:
        return "No Horizon Active / Low Risk"
    if active_horizons == [120]:
        return "Only H=120 Active / Early Warning Stage"
    if active_horizons == [120, 60]:
        return "H=120 and H=60 Active / Risk Increasing"
    if active_horizons == [120, 60, 30]:
        return "H=120, H=60, and H=30 Active / Failure Imminent"

    return f"Intermediate Stage: active horizons = {active_horizons}"
...
def _write_ttf_table(df_test, HLIST, ts_paths, te_events, intent, out_csv):
    ...
    for H, path in ts_paths.items():
        df = pd.read_csv(path)
        t = df['t'].values
        r = df[f'ema_{intent}'].values
        tau = df[f'tau_{intent}'].values[0]
    ...
    out_df = pd.DataFrame(rows).sort_values('H', ascending=False)
    out_df.to_csv(out_csv, index=False)
    return out_df, e

def _write_ttf_table_old(df_test, HLIST, ts_paths, te_events, intent, out_csv):
    e = _pick_event_for_intent(te_events, intent, 'singleton')
    if e is None: return
    s0, f0, e0 = e['start'], e['failure_time'], e['end']
...
def _stage_label(active_horizons):
    ...
    if len(active_horizons) == 0:
        return "No Horizon Active\nLow Risk"
    if len(active_horizons) == 1:
        return f"Only H={active_horizons[0]} Active\nEarly Warning Stage"
    if len(active_horizons) == 2:
        return f"H={active_horizons[0]} and H={active_horizons[1]} Active\nRisk Increasing"
    if len(active_horizons) >= 3:
        return f"H={active_horizons[0]}, H={active_horizons[1]}, and H={active_horizons[2]} Active\nFailure Imminent"

def _plot_operational_stage(ttf_csv, out_png, intent):
    ttf = pd.read_csv(ttf_csv).sort_values("H", ascending=False)

    active = ttf.loc[ttf["t_alert"].notna(), "H"].astype(int).tolist()
    stage = _stage_label(active)
    ...
    fig.text(0.50, 0.74, f"Intent: {intent}", ha="center", va="center",
             fontsize=14, fontweight="bold")
    fig.text(0.50, 0.67, stage, ha="center", va="center", fontsize=14)

    # Bottom timeline
    ax = fig.add_axes([0.08, 0.15, 0.84, 0.20])
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.spines["left"].set_visible(False)
    ax.get_yaxis().set_visible(False)

    max_ttf = max(float(ttf["est_TTF_min"].fillna(0).max()), 1.0)
    ax.set_xlim(max_ttf + 5, -5)   # reverse: large TTF on left, failure on right
    ax.axhline(0, color="black", linewidth=2)

    for _, row in ttf.iterrows():
        if pd.isna(row["t_alert"]):
            continue
        x = float(row["est_TTF_min"])
        H = int(row["H"])
        ax.scatter(x, 0, s=250, zorder=3)
        ax.text(x, 0.12, f"H={H}", ha="center", va="bottom", fontsize=11)

    ax.text(-1, -0.20, "Failure", ha="left", va="top",
            fontsize=12, fontweight="bold")

    ax.set_xlabel("Estimated time-to-failure (min)")
...
def _shap_for_alert(model, scaler, Xte, Tte, df_test, ts_path, intent, feature_names,
    ...
    # ------------- waterfall plot (same SHAP values, different style) -------------
    ...
    # make the plot a bit cleaner for paper use
    plt.tight_layout()
    plt.savefig(out_png, dpi=dpi, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    ttf_df, e = _write_ttf_table(
        df_te, args.horizons, ts_paths, te_ev, args.intent,
        os.path.join(args.outdir, f'ttf_table_{args.fold}_{args.intent}.csv')
    )

    if ttf_df is not None and e is not None:
        active_horizons = ttf_df.loc[ttf_df['t_alert'].notna(), 'H'].astype(int).tolist()
        stage = _stage_label(active_horizons)
        ...
        summary = {
            ...
            'end': e['end'],
            'stage': stage,
            'active_horizons': ','.join(map(str, sorted(active_horizons, reverse=True))),
            'n_active_horizons': len(active_horizons),
        }

        # add per-horizon details into one row
        for _, row in ttf_df.iterrows():
            H = int(row['H'])
            summary[f'H{H}_t_alert'] = row['t_alert']
            summary[f'H{H}_threshold'] = row['threshold']
            summary[f'H{H}_risk_at_alert'] = row['risk_at_alert']
            summary[f'H{H}_est_TTF_min'] = row['est_TTF_min']

        pd.DataFrame([summary]).to_csv(
            os.path.join(args.outdir, f'instance_stage_summary_{args.fold}_{args.intent}.csv'),
            index=False
        )


    ttf_csv = os.path.join(args.outdir, f"ttf_table_{args.fold}_{args.intent}.csv")
    _write_ttf_table(df_te, args.horizons, ts_paths, te_ev, args.intent, ttf_csv)

    _plot_operational_stage(
        ttf_csv,
        os.path.join(args.outdir, f"operational_stage_{args.fold}_{args.intent}.png"),
        args.intent
    )
